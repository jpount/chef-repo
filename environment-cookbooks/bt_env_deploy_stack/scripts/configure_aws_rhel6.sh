...
mkfs.ext4 -m1 /dev/mapper/vg_da1-lv_orabin
mkfs.ext4 -m1 /dev/mapper/vg_da1-lv_XXX
mkdir /oraXXX
mkdir /orabin
cat << EOF >> /etc/fstab 
/dev/mapper/vg_da1-lv_XXX /oraXXX               ext4    defaults        1 2
/dev/mapper/vg_da1-lv_orabin /orabin                 ext4    defaults        1 2
EOF
mount -a


# Config files
cat << EOF > /etc/auto.adaimnt
#
...
yumaction install samba --enablerepo=centos
...
echo $(ifconfig eth0|grep "inet addr"|awk '{print $2}'|cut -f 2 -d :) XXX >> /etc/hosts
reboot
